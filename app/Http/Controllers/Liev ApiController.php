...
class ApiController extends BaseController
{
            ...
            foreach ($locations as $location) {
                ...
                if ($safeZones->isNotEmpty()) {
                    ...
                    foreach ($safeZones as $safeZone) {
                        ...
                        // Log::info('zone noti :', ['user_id' => $user->id,'distance'=>$distance, 'zone_meter'=> $safeZone->zone_meter]);
    ...
    public function zone_list(Request $request)
    {
        try {
            ...
            return $this->sendResponse($encryptedResponse, 'Zone list retrieved successfully.');
    ...
    public function user_hold_location_details_same(Request $request)  # with same location
    {
        try {
            $request->validate([
                'user_id' => 'required', // child user id
    ...
    public function remove_user_history(Request $request)
    {
        ...
        Log::info('remove_user_history....', ['params_Data'=>$request->all()]);
            
        try {
            $request->validate([
                'child_user_id' => 'required',
                // 'today_date' => 'required'       // today to previous all
                // 'start_date' => 'required',      // start to end date only
                // 'end_date' => 'required'
            ]);
            $user = Auth::user();
            if (!$user) {
    ...
    public function get_home_data(Request $request)
    {
        $user = Auth::user();
        if (!$user) {
            //  Log::warning('Invalid token or user not found.', ['request_data' => $request->all()]);
            return $this->sendError("Authentication failed! The provided token is invalid, and the specified user could not be located.", 401);
        }
        try {
            // Log::info('Fetching home data for user', ['user_id' => $user->id]);
            $join_user_ids = DB::table('join_user')->where(['parent_user_id'=>$user->id,'is_deleted' => 0])->pluck('child_user_id')->toArray();
            //  Log::debug('Child user IDs retrieved', ['user_id' => $user->id, 'child_user_ids' => $join_user_ids]);
            if (!empty($join_user_ids)) {
                ...
                //  Log::info('Latest location history fetched', ['user_id' => $user->id, 'records_count' => $latestHistory->count()]);
            } else {
                // Handle the case where there are no child_user_ids
                // Log::info('No child users found for the parent user', ['user_id' => $user->id]);
            ...
            foreach ($latestHistory as $item) {
                ...
                // dd($removal,$removal->history_remove_start_date, $item->datetime,$item->datetime < $removal->history_remove_start_date);
                //   // Include only if both dates exist and $item->datetime is outside that range
                ...
                // dd($remove_date,$item->datetime, $item->datetime >= $remove_date);
                // 2025-01-14T11:23:00.123Z >= 2025-04-13T13:23:00.123Z  
                ...
                if (count($result) >= 11) {
                    // Log::info('Result limit reached (11 entries)', ['user_id' => $user->id]);
                    break;
                }
            }
            
            $noti_count = DB::table('user_notifications')->where(['receiver_user_id'=>$user->id])->count();
            // Log::info('Notification count retrieved', ['user_id' => $user->id, 'noti_count' => $noti_count]);
            ...
            $encryptedResponse = $this->encryptData($response);
            //  Log::info('Home data retrieved successfully', ['user_id' => $user->id]);

            return $this->sendResponse($encryptedResponse, 'User data get successfully');
        } catch (\Exception $e) { 
            // Log::error('An unexpected error occurred', ['user_id' => $user->id, 'error' => $e->getMessage()]);
    ...
    /*public function update_location(Request $request)  # queue notification (outside zone)
    {
        ...
        try {
            //  Log::info('Update location request initiated.', ['request' => $request->all()]);
            ...
            if ($user) {
                ...
                unset($data['today_date']);
                LocationHistoryModel::create($data);
                //  Log::info('Location history saved.', ['data' => $data]);
                # USER STATUS UPDATE
                $join_status = JoinUserModel::where(['child_user_id'=>$user->id,'is_deleted'=>0])->get();
                foreach ($join_status as $key => $value1) {
                    $value1->update(['phone_battery_status'=>$request->phone_battery_status]);
                }
                //  Log::info('User status updated for joined users.', ['user_id' => $user->id]);
                ...
                if ($geojsonDataList->isNotEmpty()) {
                    ...
                    foreach ($geojsonDataList as $geojsonData) {
                        ...
                        //     Log::info('Outside zone notification dispatched.', [
                        //     'child_user_id' => $user->id,
                        //     'parent_user_id' => $parentUserId,
                ...
                // Log::info('Location update process completed successfully.', ['response' => $encryptedResponse]);

                return $this->sendResponse($encryptedResponse, 'User location updated successfully');
            }
            //  Log::warning('Authentication failed! The provided token is invalid, and the specified user could not be located.');
            return $this->sendError("Authentication failed! The provided token is invalid, and the specified user could not be located.");
        } catch (ValidationException $e) {
    ...
    # multiple location data store from array
    public function update_location_red(Request $request)  # Queue notification (outside zone,safe zone)
    {
        try {
            // Log::info('Location update request received', ['request_data' => $request->all()]);
            $request->validate([
            ...
            foreach ($locations as $location) {
                ...
                if ($safeZones->isNotEmpty()) {
                    ...
                    if (!empty($safeParentUserId)) {
                         ...
                         Log::info('Safe zone noti :', ['user_id' => $user->id]);
                    ...
                    // $parentUserId = [1]; // testing
                    
                    if (!empty($parentUserId)) {
                         Log::info('OUTSIDE zone noti :', ['user_id' => $user->id]);
                ...
                # OUTSIDE ZONE NOTIFICATION FROM GEOJSON DATA
                /*$geojsonDataList = GeoJson::where('child_user_id', $user->id)->where('noti_status', 'on')->get();
                if ($geojsonDataList->isNotEmpty()) {
                    foreach ($geojsonDataList as $geojsonData) {
                        $geojsonString = is_array($geojsonData->geojson) ? json_encode($geojsonData->geojson) : $geojsonData->geojson;
                        ...
                        if (!$geometry->is_inside) {
                            // Log::warning('User outside restricted area', ['user_id' => $user->id]);
                            ...
                            // Dispatch notification for each parent user
                            SendNotificationJob::dispatch($user, $parentUserId,$noti_data);
                            // Log::info('Notification dispatched', ['user_id' => $user->id, 'parent_id' => $parentUserId]);
                        }
                    }
                }*/
            }
    ...
    public function user_location_details_single_zone(Request $request)  # with hold status and geo-json data
    {
        try {
            $request->validate([
                'user_id' => 'required', // child user id
